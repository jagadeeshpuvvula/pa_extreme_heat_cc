...
hazard_sf <- st_as_sf(
  hazard_anomalies_df,
  coords = c("x", "y"),  # longitude and latitude columns
  crs = 4326,  # WGS84 coordinate system (standard lat/lon)
  remove = FALSE  # Keep x and y columns in the data
)

result_25 <- optimized_hotspot_analysis(hazard_sf, chunk_size = 100000, k = 25)

save(result_25, file = "D:/cc_vulnerability/co_exp_hazard/hot_spot_results_k25.rda")
```

#visualize hotspot analysis results
```{r}
load("D:/cc_vulnerability/co_exp_hazard/hot_spot_results_k25.rda")
result<- result_25
...
ggsave(
  filename = "D:/cc_vulnerability/hazard_hot_spot_rev_k25.tiff",
  plot = p1,
  width = 8,
  height = 7,
  dpi = 300,
...
legend <- bi_legend(
    ...
    ylab = "Population vulnerability",
    size = 8,
    pad_width = 0.2,
    pad_color = "white"
)

# Combine map + inset legend into one figure
final_plot <- ggdraw() +
    draw_plot(map, x = 0, y = 0, width = 1, height = 1) +
    draw_plot(legend, x = 0.06, y = 0.08, width = 0.22, height = 0.22)

# Save as 300 dpi TIFF
ggsave(
    filename = "D:/cc_vulnerability/risk_map_fin/risk_map_final_x.tiff",
    plot = final_plot,
    device = "tiff",
    width = 12,
    height = 9,
...
ggplot() +
  geom_tile(
    data = extra_plot, 
    aes(x = X, y = Y, fill = bi_class, colour = "royalblue"), 
    show.legend = FALSE
  ) +
  geom_sf(
    data = conus_sf, 
...
ggsave("D:/cc_vulnerability/extra_plot.tiff", 
       plot = last_plot(), 
