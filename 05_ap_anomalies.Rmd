...
omaha_sf <- places(state = "NE", cb = TRUE) |>
  filter(NAME == "Omaha") |>
  st_transform(4326)

pm <- rast("D:/cc_vulnerability/pm_anomalies/pm25_exceedance_days_2010_2020.tif")
bc <- rast("D:/cc_vulnerability/bc_anomalies/bc_exceedance_days_2010_2020.tif")
...
make_df <- function(r){
  ...
  r |>
    ...
    mutate(decile = factor(ntile(value, 10), labels = paste0("D", 1:10)))
}
...
p1 <- ggplot(pm_df) +
  ...
  geom_sf(data = omaha_sf, fill = NA, color = "white", linewidth = 1) +
