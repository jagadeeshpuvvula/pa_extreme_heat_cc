...
ped_pop_blk_grp_aggr <- ped_pop_blk_grp |>
  ...
  select(-c("geometry")) |>
  group_by(name) |>
  summarize(total_un_empl_hsb_e = sum(un_empl_hsb_e))
```
...
pop_df<- ped_pop |>
  ...
  mutate(blk_pct = sprintf("%.2f", (blk_pop / (m_pop + f_pop)) * 100),
       ...
       un_empl_pct = sprintf("%.2f", (total_un_empl_hsb_e / households) * 100),
...
```{r}
save(pop_df, file = "~/Documents/extreme_heat_cc/data/acs_data_extract.RDA")
```


```{r}
pa_tracts <- tracts(state = "PA")
nj_tracts <- tracts(state = "NJ")
de_tracts <- tracts(state = "DE")

# Combine the three state shapefiles
combined_tracts <- rbind(pa_tracts, nj_tracts, de_tracts) |> 
  clean_names() |>
  as_tibble()

dat_fin <- right_join(combined_tracts, pop_df, by = "geoid")
```

#export data
```{r}
save(dat_fin, file="~/Documents/extreme_heat_cc/data/SDOH_final.RDA")
```
